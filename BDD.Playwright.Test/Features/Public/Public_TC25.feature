...
@tag1
Scenario: Public_TC25
	Given User logs into GBIZ
	When Verifying the Home Page Title
	When User clicks on Make a Payment option from the toolbar
	And User enters policy details and submit button on Make a Payment page from "MakeAclaimStartTC25"
    When User clicks on submit with index 2
	And User selects payment type and plan then enters payment and banking and insured information then proceeds to review from "MakeAPaymentTC25"
